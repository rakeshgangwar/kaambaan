    ...
    <div class="bg-surface border-border drawer-in relative flex max-h-[90vh] w-full max-w-lg flex-col rounded-[12px] border shadow-2xl">
      ...
      <div class="flex-1 space-y-6 overflow-y-auto p-5">
        ...
        <section>
          ...
          <p class="text-muted-foreground mb-2 text-xs leading-relaxed">Add this URL as a webhook in your repo (Settings → Webhooks), with the secret below. Turn on the trigger to open a card for each new issue.</p>
          <div class="bg-inset border-border mono mb-2.5 flex items-center justify-between gap-2 overflow-hidden rounded-[7px] border px-2.5 py-1.5 text-[10px]">
          ...
          <input
            ...
            type="password"
            placeholder={board.github.webhookConfigured ? 'webhook secret (set — type to replace)' : 'webhook secret'}
            class="bg-inset border-border focus:border-marigold mb-2.5 w-full rounded-[7px] border px-3 py-2 text-sm outline-none"
          />
